...
local function path_widget(root, fname)
  ...
  if bo.buftype == "help" then return HL.file .. path end
  ...
  if remote and branch then
    ...
    repo_info = string.format("%s %s @ %s ", HL.branch, remote, branch)
  end
  ...
  return repo_info .. HL.file .. " " .. dir_path .. path .. " "
end
...
function M.render()
  ...
  local parts = {
    ...
    mod = get_opt("modifiable", { buf = buf })
        and (get_opt("modified", { buf = buf }) and HL.modified or " ")
      or HL.nomodifiable,
    ro = get_opt("readonly", { buf = buf }) and HL.readonly or "",
    sep = SEP,
